...
the same `update.sh` a person would. The VPN is the one job that takes data:
gluetun's settings, which the helper checks against a fixed list of variables
and writes to `.env.vpn` itself. The docker socket stays where it is: handing
that to a container is handing it the machine.

A helper from before 0.9.3 cannot update itself yet. Running
`cd /opt/uwumail && sudo bash update.sh` once brings UwUMail and the helper up
to date; from then on both are updated from the portal, and `update.sh` always
takes the helper along. The portal shows that command until it is done.
