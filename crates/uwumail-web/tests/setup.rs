...
async fn a_fresh_server_is_set_up_with_the_code_from_the_log() {
    ...
    assert_eq!((status["open"].clone(), status["hostname"].clone()), (json!(true), json!("mail.example.org")));
    ...
    assert_eq!(store.dkim_keys("example.org").await.unwrap().len(), 2, "the domain got its keys");
    ...
    let nyu = store.account("nyu@example.org").await.unwrap().unwrap();
    let reply =
