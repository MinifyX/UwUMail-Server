...
use uwumail_gateway::logs::{KEPT_LINES, LogQueue};
...
async fn main() -> ExitCode {
    ...
    let logs = init_logging(&config, matches!(command, Command::Serve));

    let result = match command {
        Command::Serve => serve(config, logs).await,
...
fn init_logging(config: &GatewayConfig, serving: bool) -> Option<Arc<LogQueue>> {
    ...
    // What the gateway says while it runs also goes to the server, for its portal and its logs.
    let logs = serving.then(|| LogQueue::new(KEPT_LINES));
    tracing_subscriber::registry().with(filter).with(output).with(logs.as_ref().map(LogQueue::layer)).init();
    logs
}

async fn serve(config: GatewayConfig, logs: Option<Arc<LogQueue>>) -> anyhow::Result<()> {
    tracing::info!(version = env!("CARGO_PKG_VERSION"), "UwUMail Gateway is waking up (=^･ω･^=)");
    let (shutdown, shutdown_rx) = watch::channel(false);
    let running = uwumail_gateway::start_with_logs(config, shutdown_rx, logs).await?;
